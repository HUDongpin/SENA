...
- Multi-value delimiter contract: `docs/adr/0007-person-id-multivalue-delimiter-contract.md`. Multi-value contract cells (`codes`, `target_person_ids`) separate values with `|` only; a cell with no `|` is one value, verbatim, so comma-bearing ids like `"Wong, Ka Yee"` round-trip. JSON-contract arrays are the canonical escape. Ids must not contain `|`; `,`/`;` inside an id are tolerated with a cleaning-manifest warning.
- Actor typing contract: `docs/adr/0006-forum-reply-bridge-evidence-and-actor-typing.md` D2 (Track C-P0). The roster's optional `actor_type` marks a row `human` (default when absent) or `ai_agent`; `target_actor_ids` is an import alias of `target_person_ids`. Typing is additive roster semantics only — no matrix behavior reads it, an AI row raises a cleaning-manifest disclosure, and no Human–AI research claim follows from putting AI in `people`.
- AI-run provenance contract: `docs/adr/0013-ai-agent-runs-provenance.md` (Track C-P0). Optional additive `ai_agent_runs` records provider / model_family / model_snapshot / deployment_id / api_version / agent_config_version / system_prompt_hash / sampling params / started_at / ended_at, with explicit `unknown` or `not_exposed` when a field is unavailable. Absence must not break five-table v1 import. Missing run provenance is not optional when AI actors are present in research claims. The event ledger (C-P1) remains a future ADR.
