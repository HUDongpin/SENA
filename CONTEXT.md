...
- Actor typing contract: `docs/adr/0006-forum-reply-bridge-evidence-and-actor-typing.md` D2 (Track C-P0). The roster's optional `actor_type` marks a row `human` (default when absent) or `ai_agent`; `target_actor_ids` is an import alias of `target_person_ids`. Typing is additive roster semantics only — no matrix behavior reads it, an AI row raises a cleaning-manifest disclosure, and no Human–AI research claim follows from it (run provenance and the event ledger are future ADRs).
