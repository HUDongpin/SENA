...
function hasOnlyCarrierKeys(
  ...
  const maximumAllowedKeyLength = allowedKeys.reduce((maximum, key) => Math.max(maximum, key.length), 0);
  // Reserve the complete bounded carrier operation before inspecting an
  // untrusted object. A request with only one unit left therefore cannot force
  // prototype, proxy, key, or descriptor work for another holder.
  traversalBudget?.reserveWork(allowed.size + 2);
  if (carrierRuntimeIdentifiesProxy(value)) return false;
  ...
  if (ownKeys.length > allowed.size) {
    // Reflect.ownKeys is necessarily one operation in JavaScript, but charge
    // every unexpected key immediately so the same request cannot repeat a
    // large rejected enumeration.
    traversalBudget?.reserveWork(ownKeys.length - allowed.size);
    return false;
  }
  for (const key of ownKeys) {
    if (typeof key !== "string") return false;
    // Membership is decided before any UTF scan. Exact allowed keys are trusted
    // schema constants; an arbitrarily long unknown key is rejected from its
    // O(1) length before Set lookup or string validation.
    if (key.length > maximumAllowedKeyLength || !allowed.has(key)) return false;
...
function hasExactCarrierKeys(
  ...
  if (!hasOnlyCarrierKeys(value, [...requiredKeys, ...optionalKeys], traversalBudget)) return false;
...
function isBoundedDenseCarrierArray(
  ...
  // Pre-reserve the complete dense-index descriptor pass before materializing
  // keys. This prevents a nearly exhausted request from starting an O(n) scan.
  traversalBudget?.reserveWork((length as number) + 2);
  let ownKeys: PropertyKey[];
  try {
    ownKeys = Reflect.ownKeys(value);
  } catch {
  ...
  if (ownKeys.length > (length as number) + 1) {
    traversalBudget?.reserveWork(ownKeys.length - ((length as number) + 1));
    return false;
  }
  let ownEntryCount = 0;
  const maximumIndexKeyLength = Math.max(1, String(Math.max(0, maximumEntries - 1)).length);
  for (const key of ownKeys) {
    if (key === "length") continue;
    if (typeof key !== "string") return false;
    if (key.length > maximumIndexKeyLength) return false;
...
type SenaGroupComparisonSourceAdmission = {
  sourceKey: string;
  peopleCount: number;
  canonicalSource: SenaGroupComparisonSourceContext;
  buildOptions?: Partial<SenaBuildOptions>;
  sourceModelWorkUnits: number;
};
...
const sourceDigestTextEncoder = new TextEncoder();

function sourceDigestUtf8ByteLength(value: string) {
  assertWellFormedSourceDigestString(value);
  return sourceDigestTextEncoder.encode(value).byteLength;
}

function nodeRuntimeSha256Hash() {
  const runtimeProcess = (globalThis as typeof globalThis & {
    process?: { getBuiltinModule?: (id: string) => unknown };
  }).process;
  const crypto = runtimeProcess?.getBuiltinModule?.("node:crypto") as {
    createHash?: (algorithm: string) => {
      update: (value: string, encoding?: "utf8") => void;
      digest: (encoding: "hex") => string;
    };
  } | undefined;
  const hash = crypto?.createHash?.("sha256");
  if (!hash) {
    throw new Error("SENA group-comparison source verification requires the Node SHA-256 runtime.");
  }
  return hash;
}
...
function measureSenaGroupComparisonSourceDigest(
  ...
  onWorkUnitAttempt: (units?: number) => void,
  onDigestBytesAttempt: (bytes: number) => void
  ...
  const reserveTraversalWork = (units = 1) => {
    onWorkUnitAttempt(units);
    if (!Number.isSafeInteger(units) || units < 1 ||
      units > maximumWorkUnits - measurement.workUnits) {
      throw new Error("SENA group-comparison source digest scan budget exceeded.");
    }
    measurement.workUnits = safeSourceDigestAdd(measurement.workUnits, units);
  };
  const reserveDigestText = (value: string, countsAsSourceText: boolean) => {
    ...
    // UTF validation is itself linear in the source string. Charge its
    // conservative code-unit lower bound before scanning so a malformed final
    // surrogate cannot be retried for free. Add only the multibyte delta after.
    onDigestBytesAttempt(value.length);
    const bytes = sourceDigestUtf8ByteLength(value);
    if (bytes > value.length) onDigestBytesAttempt(bytes - value.length);
  ...
  const visit = (value: unknown) => {
    ...
    try {
      if (Array.isArray(value)) {
        if (carrierRuntimeIdentifiesProxy(value)) invalidSourceDatasetCarrier();
        ...
        // Charge the bounded container step before descriptor materialization;
        // an exhausted request never enumerates a dense maximum-size array.
        reserveTraversalWork((length as number) + 2);
        const descriptors = carrierOwnDataDescriptors(value, Array.prototype);
        if (!descriptors || Reflect.ownKeys(descriptors).length !== (length as number) + 1) {
          invalidSourceDatasetCarrier();
        }
      ...
      // Object descriptors and key ordering are structural digest work too.
      // Charge them before either operation so a failed prior traversal cannot
      // repeatedly force enumeration or sorting without consuming budget.
      reserveTraversalWork();
...
function senaGroupComparisonSourceDigest(source: SenaGroupComparisonSourceContext) {
  const hash = nodeRuntimeSha256Hash();
  const ancestors = new WeakSet<object>();
...
function projectSourceCarrierObject(
  value: unknown,
  requiredKeys: readonly string[],
  optionalKeys: readonly string[] = [],
  traversalBudget?: SenaGroupComparisonCarrierTraversalBudget
) {
  if (typeof value !== "object" || value === null) invalidSourceDatasetCarrier();
  ...
  // Reserve proxy/prototype work and every known descriptor read before the
  // first reflective operation. Unknown own properties are deliberately never
  // enumerated: only this schema projection becomes the canonical carrier.
  traversalBudget?.reserveWork(knownKeys.length + 3);
  if (carrierRuntimeIdentifiesProxy(value)) invalidSourceDatasetCarrier();
  ...
  if (prototype !== Object.prototype && prototype !== null) invalidSourceDatasetCarrier();
  ...
  for (const key of knownKeys) {
    ...
    if (!descriptor) {
      if (requiredKeys.includes(key)) invalidSourceDatasetCarrier();
      continue;
    }
...
function projectSourceCarrierArray<T>(
  value: unknown,
  minimumEntries: number,
  maximumEntries: number,
  projectEntry: (entry: unknown) => T,
  traversalBudget?: SenaGroupComparisonCarrierTraversalBudget
) {
  let arrayCarrier = false;
  try {
  ...
  if (!arrayCarrier) invalidSourceDatasetCarrier();
  // Two or fewer remaining units fail before proxy/prototype/length touches.
...
function projectSourceMetadataCarrier(
  value: unknown,
  traversalBudget?: SenaGroupComparisonCarrierTraversalBudget
) {
  const metadata = projectSourceCarrierObject(value, [
    "datasetVersion",
    "consent",
    "retention",
    "pseudonymization",
  ...
  return {
    ...
    consent: projectSourceCarrierObject(metadata.consent, ["instrument", "date", "scope"], [], traversalBudget),
    retention: projectSourceCarrierObject(metadata.retention, ["policy"], ["deleteBy"], traversalBudget),
    pseudonymization: projectSourceCarrierObject(
      metadata.pseudonymization,
      ["personIdPolicy", "rosterMapping"],
      [],
      traversalBudget
    ),
    codebook: projectSourceCarrierObject(metadata.codebook, ["id", "version", "contentHash"], [], traversalBudget)
  } as SenaDataset["metadata"];
}

function projectSourceBuildOptionsCarrier(
  value: unknown,
  traversalBudget?: SenaGroupComparisonCarrierTraversalBudget
) {
  if (value === undefined) return undefined;
  const projected = projectSourceCarrierObject(value, [], sourceBuildOptionKeys, traversalBudget);
  if (Object.hasOwn(projected, "temporal") && projected.temporal !== undefined) {
    projected.temporal = projectSourceCarrierObject(projected.temporal, [], [
      "mode",
      "movingWindowSize",
      "movingWindowStep",
      "turnWindowRadius"
  ...
  return projected as Partial<SenaBuildOptions>;
}

function projectSourceDatasetRows<T>(
  rows: unknown,
  requiredKeys: readonly string[],
  optionalKeys: readonly string[] = [],
  projectRow?: (row: Record<string, unknown>) => T,
  traversalBudget?: SenaGroupComparisonCarrierTraversalBudget
) {
  return projectSourceCarrierArray(
    rows,
    0,
    SENA_GROUP_COMPARISON_SOURCE_MODEL_MAX_COLLECTION_ENTRIES,
    (entry) => {
      const row = projectSourceCarrierObject(entry, requiredKeys, optionalKeys, traversalBudget);
      return projectRow ? projectRow(row) : row as T;
    },
    traversalBudget
  );
}

function projectSenaGroupComparisonSourceContextCarrier(
  source: SenaGroupComparisonSourceContext,
  traversalBudget?: SenaGroupComparisonCarrierTraversalBudget
) {
  const holder = projectSourceCarrierObject(source, ["dataset"], ["buildOptions"], traversalBudget);
  ...
  const people = projectSourceDatasetRows(
    ...
    undefined,
    traversalBudget
  ) as SenaDataset["people"];
  const interactions = projectSourceDatasetRows(dataset.interactions, [
    "source",
    "target",
    "channel",
    "stage",
    "evidence"
  ], ["weight", "turnIndex"], undefined, traversalBudget) as SenaDataset["interactions"];
  const utterances = projectSourceDatasetRows(dataset.utterances, [
    "id",
    "personId",
    "unitId",
    "stanzaId",
    ...
    "turnIndex",
    "text"
  ], ["timestamp"], undefined, traversalBudget) as SenaDataset["utterances"];
  const codedSegments = projectSourceDatasetRows(dataset.coded_segments, [
    "segmentId",
    "utteranceId",
    "personId",
    "unitId",
    ...
    row.codes = projectSourceCarrierArray(
      row.codes,
      0,
      SENA_GROUP_COMPARISON_SOURCE_MODEL_MAX_COLLECTION_ENTRIES,
      (entry) => entry,
    ...
    if (Object.hasOwn(row, "targetPersonIds") && row.targetPersonIds !== undefined) {
      row.targetPersonIds = projectSourceCarrierArray(
        row.targetPersonIds,
        0,
        SENA_GROUP_COMPARISON_SOURCE_MODEL_MAX_COLLECTION_ENTRIES,
        (entry) => entry,
    ...
    return row;
  }, traversalBudget) as SenaDataset["coded_segments"];
  const codebook = projectSourceDatasetRows(
    dataset.codebook,
    ["id", "label", "family", "description", "color"],
    [],
    undefined,
    traversalBudget
  ) as SenaDataset["codebook"];
  const projectedDataset: SenaDataset = {
    people,
    interactions,
    utterances,
  ...
  if (Object.hasOwn(dataset, "metadata")) {
    projectedDataset.metadata = dataset.metadata === undefined
      ? undefined
      : projectSourceMetadataCarrier(dataset.metadata, traversalBudget);
  }
  if (Object.hasOwn(dataset, "warnings")) {
    projectedDataset.warnings = dataset.warnings === undefined
      ? undefined
      : projectSourceCarrierArray(
        dataset.warnings,
        0,
        SENA_GROUP_COMPARISON_SOURCE_MODEL_MAX_COLLECTION_ENTRIES,
        (entry) => entry,
  ...
  const buildOptions = Object.hasOwn(holder, "buildOptions")
    ? projectSourceBuildOptionsCarrier(holder.buildOptions, traversalBudget)
    : undefined;
  return {
    source: buildOptions === undefined
      ? { dataset: projectedDataset }
      : { dataset: projectedDataset, buildOptions },
...
export function estimateSenaGroupComparisonSourceModelWorkUnits(
  ...
  const projected = projectSenaGroupComparisonSourceContextCarrier(source);
  return estimateAdmittedSenaGroupComparisonSourceModelWorkUnits(projected.source, maximumWorkUnits);
}

function estimateAdmittedSenaGroupComparisonSourceModelWorkUnits(
  source: SenaGroupComparisonSourceContext,
  maximumWorkUnits: number,
  attemptBudget?: {
    reserveWork: (units?: number) => void;
    reserveTextBytes: (bytes: number) => void;
  }
) {
  ...
  const reserveSourceText = (value: unknown) => {
    ...
    attemptBudget?.reserveWork(1);
    attemptBudget?.reserveTextBytes(value.length);
    assertWellFormedSourceDigestString(value);
    const bytes = sourceDigestTextEncoder.encode(value).byteLength;
    if (bytes > value.length) attemptBudget?.reserveTextBytes(bytes - value.length);
  ...
  for (const segment of dataset.coded_segments) {
    const codeReferences = segment.codes;
    const targets = segment.targetPersonIds ?? [];
    const nestedEntries = safeAdmissionWorkSum([codeReferences.length, targets.length]);
  ...
  for (const rowsWithTurns of [dataset.utterances, dataset.coded_segments]) {
    if (rowsWithTurns.length > 0) attemptBudget?.reserveWork(rowsWithTurns.length);
  ...
  if (source.buildOptions?.temporal?.mode === "stage") {
    ...
    for (const rowsWithStage of [dataset.utterances, dataset.coded_segments, dataset.interactions]) {
      if (rowsWithStage.length > 0) attemptBudget?.reserveWork(rowsWithStage.length);
...
/**
 * Turn-scoped verifier for content-bound holder datasets and build options.
 *
 ...
 * read/create/review turn. Every supplied holder is re-admitted and re-hashed;
 * model/evidence reuse occurs only after the current content digest is known.
 */
export class SenaGroupComparisonSourceVerificationCache {
  ...
  private sourceDigestBytesAttempted = 0;
  ...
  get sourceDigestMeasurementBytesAttempted() {
    return this.sourceDigestBytesAttempted;
  }
  ...
  private admitSourceContext(source: SenaGroupComparisonSourceContext): SenaGroupComparisonSourceAdmission {
    if (this.sourceDigestBytesAttempted >= this.maximumSourceDigestBytes ||
      this.sourceDigestMeasurementAttempts >= this.maximumSourceDigestWorkUnits) {
      throw new Error("SENA group-comparison source digest scan budget exceeded.");
    }
    const attemptedWorkAtStart = this.sourceDigestMeasurementAttempts;
    const reserveAttemptedWork = (units = 1) => {
      if (!Number.isSafeInteger(units) || units < 1) {
        throw new Error("SENA group-comparison source digest scan budget is not safely representable.");
      }
      if (units > this.maximumSourceDigestWorkUnits - this.sourceDigestMeasurementAttempts) {
        this.sourceDigestMeasurementAttempts = this.maximumSourceDigestWorkUnits;
        throw new Error("SENA group-comparison source digest scan budget exceeded.");
      }
      this.sourceDigestMeasurementAttempts += units;
    };
    const reserveAttemptedBytes = (bytes: number) => {
      if (!Number.isSafeInteger(bytes) || bytes < 0) {
        throw new Error("SENA group-comparison source digest scan budget is not safely representable.");
      }
      if (bytes > this.maximumSourceDigestBytes - this.sourceDigestBytesAttempted) {
        this.sourceDigestBytesAttempted = this.maximumSourceDigestBytes;
        throw new Error("SENA group-comparison source digest scan budget exceeded.");
      }
      this.sourceDigestBytesAttempted += bytes;
    };
    // Schema projection is part of the same request-wide work budget as
    // measurement and hashing. It reads only known fields and declared dense
    // indices, so arbitrary extra own keys never need an unbounded enumeration.
    const traversalBudget = { reserveWork: reserveAttemptedWork };
    const projected = projectSenaGroupComparisonSourceContextCarrier(source, traversalBudget);
    const { source: canonicalSource, peopleCount } = projected;
    const sourceModelWorkUnits = estimateAdmittedSenaGroupComparisonSourceModelWorkUnits(
      canonicalSource,
      this.maximumSourceModelWorkUnits,
      {
        reserveWork: reserveAttemptedWork,
        reserveTextBytes: reserveAttemptedBytes
      }
    );
    const remainingDigestBytes = this.maximumSourceDigestBytes - this.sourceDigestBytesAttempted;
    const remainingDigestWorkUnits = this.maximumSourceDigestWorkUnits -
      this.sourceDigestMeasurementAttempts;
    const measurement = measureSenaGroupComparisonSourceDigest(
      ...
      reserveAttemptedWork,
      reserveAttemptedBytes
    );
    ...
    // Failed traversals remain charged through the attempted byte/work counters.
    this.reservedSourceDigestBytes += measurement.digestBytes;
    this.reservedSourceDigestWorkUnits += this.sourceDigestMeasurementAttempts - attemptedWorkAtStart;
    const sourceKey = senaGroupComparisonSourceDigest(canonicalSource);
    this.sourceDigestScans += 1;
    return {
      sourceKey,
      peopleCount,
      canonicalSource,
      buildOptions: canonicalSource.buildOptions,
      sourceModelWorkUnits
    };
  }
  ...
  private sourceEntry(
    source: SenaGroupComparisonSourceContext,
    admittedSource?: SenaGroupComparisonSourceAdmission
  ) {
    const admission = admittedSource ?? this.admitSourceContext(source);
    const { sourceKey, canonicalSource, buildOptions } = admission;
    ...
    if (!entry) {
      ...
      const sourceModelWorkUnits = admission.sourceModelWorkUnits;
      ...
      entry = {
        model: buildSenaModel(canonicalSource.dataset, buildOptions ?? {}),
  ...
  private verifyCurrentComparisonSource(
    ...
    if (!isRecord(value) || !isRecord(value.sourceEvidence) ||
      ...
      value.sourceEvidence.metricUniverse.length !== admittedSource.peopleCount) {
    ...
    const expected = this.expectedEvidence(source, comparison, admittedSource);
  ...
  private currentResultVerificationKey(
    ...
    if (
      ...
      this.verifyCurrentComparisonSource(value, source, admittedSource);
      return resultKey;
    }
    if (
      value.schemaVersion !== SENA_SCHEMA_VERSIONS.groupComparisonSuite ||
    ...
    for (const comparison of value.comparisons) {
      this.verifyCurrentComparisonSource(comparison, source, admittedSource);
    }
    this.verifyCurrentComparisonSource(value.primary, source, admittedSource);
    return resultKey;
  }
  ...
  normalizeBoundResult(
    ...
    const admittedSource = this.admitSourceContext(source);
    assertSenaGroupComparisonValidationCarrier(value, admittedSource.peopleCount);
    const verificationKey = this.currentResultVerificationKey(value, source, admittedSource);
    const entry = verificationKey ? this.sourceEntry(source, admittedSource) : undefined;
    ...
    this.assertMatches(normalized, source, admittedSource);
  ...
  private expectedEvidence(
    ...
    const entry = this.sourceEntry(source, admittedSource);
  ...
  assertMatches(
    ...
    for (const comparison of currentComparisons) {
      const expected = this.expectedEvidence(source, comparison, admission);
...
export function normalizeSenaGroupComparisonValidationResult(
  ...
  // Result shape is the cheaper, attacker-controlled carrier. Admit it before
  // any holder digest/model work; people-cardinality binding is repeated after
  // the exact source admission inside normalizeBoundResult.
  ...
  const verificationCache = sourceVerificationCache ??
