...
- A green `npm test` does not prove the app builds: vitest never type-checks and no test imports `/workspace/ena`, which shipped a non-building `main` twice (2026-07-18, 2026-07-31). Two enforcement layers exist: `.github/workflows/build-gate.yml` runs `tsc --noEmit` + `next build` on every PR to and push of `main`, and `.githooks/pre-push` runs the same pair locally before any push of `main` (requires `git config core.hooksPath .githooks` once per clone; bypass with `--no-verify` only in emergencies). Never merge or push `main` on suite evidence alone.
