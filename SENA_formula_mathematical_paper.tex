...
elements. The proposed SENA fusion formula combines a person-person social
matrix, a code-code epistemic co-occurrence matrix, and a person-code bridge
matrix into one block matrix. This paper gives a formal mathematical analysis
...
\Af =
\begin{bmatrix}
\alpha \Shat & \gamma \Bhat \\
\gamma \Bhat^\top & \beta \What
\end{bmatrix},
\end{equation}
where \(\Shat\) is a normalized social matrix, \(\What\) is a normalized
epistemic co-occurrence matrix, \(\Bhat\) is a normalized person-code bridge
matrix, and \(\alpha,\beta,\gamma\ge 0\) are layer weights.
...
not supply the learning-analytics semantics of \(S\), \(W\), \(B\), and \(G\).
...
\begin{definition}[Bridge layer]
The bridge layer is a matrix
\[
\Bmat\in \Rnn^{n\times m},
\]
where \(B_{ia}\) denotes the contribution, use, or activation strength of code
\(c_a\) by person \(p_i\).
\end{definition}

\begin{definition}[Layer normalization]
A layer normalization is a map \(N_L:M_L\mapsto \widehat M_L\) applied
separately to \(L\in\{S,W,B\}\). Examples include
\[
...
object connecting persons to codes.
...
\(\Smat,\Wmat,\Bmat\) have nonnegative entries and
...
\Af^\top =
\begin{bmatrix}
(\alpha\Shat)^\top & (\gamma\Bhat^\top)^\top\\
(\gamma\Bhat)^\top & (\beta\What)^\top
\end{bmatrix}
=
\begin{bmatrix}
\alpha\Shat & \gamma\Bhat\\
...
=\Af,
\]
because \(\Shat=\Shat^\top\) and \(\What=\What^\top\). The block locations
...
\item If \(B=0\), the formula reduces to a disconnected social block and
epistemic block.
...
The bridge matrix \(B\) answers which person contributed to which code. ENA
...
\]
Thus \(B\) is appropriate for person-code bridge visualization, while \(G\) is
more appropriate for claims such as ``person \(i\) supported the
...
the normalization. If \(S,W,B\) are rescaled arbitrarily, then the same visual
or graph-theoretic output can be produced by compensating changes in
\(\alpha,\beta,\gamma\). Thus the parameters are not intrinsically identifiable
without a normalization convention.

\begin{proposition}[Scale dependence]
Let \(c_S,c_W,c_B>0\). Replacing \(S,W,B\) by \(c_SS,c_WW,c_BB\) and replacing
\[
\alpha,\beta,\gamma
\quad\text{by}\quad
\alpha/c_S,\ \beta/c_W,\ \gamma/c_B
\]
leaves \(\Af\) unchanged.
\end{proposition}

\begin{proof}
Substitute the rescaled matrices and rescaled weights into
the fusion matrix. Each block is unchanged.
\end{proof}
...
If \(S\ne S^\top\), then the same block construction defines a directed
heterogeneous graph rather than an undirected one. The Laplacian result above
does not apply without additional directed-graph choices or symmetrization.

A more general directed bridge model is
\[
A_{\mathrm{directed}}=
\begin{bmatrix}
\alpha\Shat & \gamma_{\mathrm{out}}\widehat B^{PC}\\
\gamma_{\mathrm{in}}\widehat B^{CP} & \beta\What
\end{bmatrix},
\]
where \(B^{PC}\) represents person-to-code activation and \(B^{CP}\) represents
code-to-person uptake, attribution, feedback, or recommendation.
...
If \(S\) contains counts in the hundreds while \(W\) and \(B\) contain binary
...
If code assignments are unstable, then \(W\), \(B\), and \(G\) inherit that
...
\item exact formulas for \(S,W,B\), and, if used, \(G\);
...
semidefinite in the undirected case, boundary cases reduce to recognizable SNA,
